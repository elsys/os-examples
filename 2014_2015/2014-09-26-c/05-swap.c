...
void swap_any(void* pa, void* pb, int size) {
	...
	for (i = 0; i < size; i++) {
		...
		c[i] = *(cpa + i); // NOTE: The syntax now is not so noisy, there are no casts. TODO: Can we do better ?
	}
	
	// *pa = *pb
	for (i = 0; i < size; i++) {
		// *((char *)pa + i) = *((char *)pb + i); // previous syntax
		
		*(cpa + i) = *(cpb + i); // NOTE: The syntax now is not so noisy, there are no casts. TODO: Can we do better ?
	}
	
	// *pb = c
	for (i = 0; i < size; i++) {
		// *((char *)pb + i) = c[i]; // previous syntax
		
		*(cpb + i) = c[i]; // NOTE: The syntax now is not so noisy, there are no casts. TODO: Can we do better ?
	}
}
